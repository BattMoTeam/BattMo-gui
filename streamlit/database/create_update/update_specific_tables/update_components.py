...
class UpdateComponents:
    ...
    def get_resource_as_json(self):
        return app_access.get_json_from_path(
            app_access.get_path_to_components()
        )
    ...
    def update_component_from_json(self, resource_file):
        ...
        for component_name in components:
            ...
            if category_id or tab_id:
                ...
                assert default_template_id is not None, "invalid default_template={} for component {}".format(
                    default_template, component_name
                )

                component_id = self.sql_component.get_id_from_name(component_name)
                ...
                print("category name = {} is not specified in the SQL table category".format(category_name))
                print("component {} has not be saved in db since category_name {} is not in db".format(component_name, category_name))
